...
escape = input("would you like to exit yes/no: ","s" )
...
for i=1:42 %this is because there are 6*7=42 possible entries, so we put the code in a big while loop to run thorough the code for each entry.
    ...
    for col=[7:-1:1]
        for row=[6:-1:1]
            ...
            for row=[6:-1:3] %index needs to be set to lower limit 3 as row-2 can take a minimum value of 1. 
            %=========verticle stack detection=========
            if board(row,col)==2 && board(row-1,col)==2 && board(row-2,col)==0  %this checks vertically to see if the user has 2 connected upwards, so that the computer can stop user from winning
                board(row-2,col)=1; %row-2 gets the row above, and places a 1. This is the computers strategy to stop the user from stacking 1s consistently upwards. 
